...
function buildItem(item) {
  ...
  if (item.special_instructions) o += "<additionalRequirements>" + X(item.special_instructions) + "</additionalRequirements>";
  ...
  if (item.is_combo && item.components && item.components.length > 0) {
    ...
    for (var c = 0; c < item.components.length; c++) {
      ...
      o += "<name>" + X(comp.component_name) + "</name>";
      ...
      if (comp.special_instructions) {
        ...
        o += "<name>" + X(comp.special_instructions) + "</name>";
        o += "<quantity>1</quantity>";
        o += "<price>0.00</price>";
        o += "</Requirement>";
      }
...
app.get("/test-order", async function (req, res) {
  ...
  var result = await sendToPOS(built.xml);
  res.json({ version: "V7-2026-03-14", xml: built.xml, pos: result, ref: built.ref });
});

app.get("/test-combo", async function (req, res) {
  var order = { order_type: "pickup", customer_name: "Test Combo", customer_phone: "8888888888",
    items: [{ item_name: '12"PIZZA W/1 TOP 8"SUB, 6 BUFFALO WING & 2 CAN OF SODA', quantity: 1, unit_price: 29.99, is_combo: true,
      components: [
...
app.get("/", function (req, res) {
  res.json({ status: "running", version: "V7-2026-03-14", restaurant_id: CONFIG.RESTAURANT_ID,
    endpoints: ["POST /retell/function/submit_order", "POST /retell/function/verify_address", "GET /test-order", "GET /test-combo", "GET /debug"] });
});

app.listen(CONFIG.PORT, function () { console.log("V7 POS Bridge | Port: " + CONFIG.PORT + " | ID: " + CONFIG.RESTAURANT_ID); });
