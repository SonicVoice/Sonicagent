...
async function sendToPOS(xml) {
  ...
  for (var a = 0; a < attempts.length; a++) {
    try {
      var r = await fetch(url, { method: "POST", headers: { "Content-Type": attempts[a].ct }, body: xml });
      var t = await r.text();
      console.log("POS attempt " + (a + 1) + ": " + r.status + " " + t);
      if (t.toLowerCase().indexOf("success") !== -1) return { ok: true, response: t };
    } catch (e) { console.log("POS attempt " + (a + 1) + " error: " + e.message); }
  }
  return { ok: false, response: "POS did not return success" };
}
