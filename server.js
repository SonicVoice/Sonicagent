...
function buildItem(item) {
  ...
  // foodMenuItemId — allows POS to map item to its menu for editing
  if (item.item_id) {
...
app.get("/test-order", async function (req, res) {
  ...
  var result = await sendToPOS(built.xml);
  res.json({ version: "V13", xml: built.xml, pos: result, ref: built.ref });
});
...
app.get("/", function (req, res) {
  res.json({ status: "running", version: "V13", restaurant_id: CONFIG.RESTAURANT_ID,
    endpoints: ["POST /retell/function/submit_order", "POST /retell/function/verify_address",
      "GET /test-order", "GET /test-combo", "GET /test-address2?q=ADDRESS", "GET /raw-xml", "GET /raw-lastorder", "GET /tag-test", "GET /debug"] });
});

app.listen(CONFIG.PORT, function () { console.log("V13 POS Bridge | Port: " + CONFIG.PORT + " | ID: " + CONFIG.RESTAURANT_ID); });
