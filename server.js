...
async function loadMenu() {
  try {
    ...
    // ── TENDERS / NUGGETS ──
    MENU.tenders = [{qty:3,id:672,price:7.49,keywords:["3 chicken tender","3 tender","3pc tender","3 piece tender"]},{qty:5,id:673,price:10.49,keywords:["5 chicken tender","5 tender","5pc tender","5 piece tender"]},{qty:7,id:674,price:12.49,keywords:["7 chicken tender","7 tender","7pc tender","7 piece tender"]}];
    MENU.nuggets = [{qty:6,id:587,price:5.99,keywords:["6 nugget","6pc nugget","6 piece nugget","6 chicken nugget"]},{qty:9,id:588,price:6.99,keywords:["9 nugget","9pc nugget","9 piece nugget"]},{qty:12,id:824,price:8.99,keywords:["12 nugget","12pc nugget","12 piece nugget"]}];
    MENU.fish_nuggets = [{qty:5,id:589,price:7.99,keywords:["5 fish nugget","5pc fish nugget"]},{qty:10,id:590,price:12.99,keywords:["10 fish nugget","10pc fish nugget"]}];
...
// ── Size resolver: "large" → 14, "medium" → 12, etc ──
function resolveSize(s) {
  if (!s) return null;
  var n = parseInt(s);
  if (n && [10,12,14,16,18,8].indexOf(n) !== -1) return n;
  var map = {"small":10,"sm":10,"10":10,"medium":12,"med":12,"12":12,"large":14,"lg":14,"lrg":14,"14":14,"xlarge":16,"x-large":16,"xl":16,"extra large":16,"16":16,"xxl":18,"xxlarge":18,"18":18,"half":8,"whole":12};
  var key = (s||"").toLowerCase().trim();
  return map[key] || null;
}

function pricePizza(size, toppings, freeToppings) {
  var sz = resolveSize(size) || 14;
...
function priceSub(subName, size, fixins, cheese, specialInstructions) {
  ...
  var sz = resolveSize(size);
  var isWhole = sz === 12 || (size||"").toLowerCase().indexOf("whole") !== -1;
...
function priceGenericItem(itemName) {
  var allLists = [
    ...
    {list: MENU.tenders, cat: "Tenders"},
    {list: MENU.nuggets, cat: "Nuggets"},
    {list: MENU.fish_nuggets, cat: "Nuggets"},
...
function processUtterance(session, input) {
  ...
  // ── GREETING ──
  if (state === "greeting") {
    // Empty utterance = agent just started, return greeting
    if (!utterance || utterance.trim() === "") {
      response.say = "Thanks for calling Demo Pizza! Is this for pickup or delivery?";
      return response;
    }
  ...
  if (state === "collecting" || state === "asking_field") {
    ...
    if (state === "asking_field" && session.current_item && session.missing_fields.length > 0) {
      ...
      if (answer || utterance) {
        ...
        if (field === "toppings") {
          ...
          if (lower.indexOf("cheese") !== -1 && (lower.indexOf("just") !== -1 || lower.indexOf("plain") !== -1 || lower === "cheese")) {
            session.current_item.toppings = [];
          } else {
            // Parse toppings from answer
            var tops = val.split(/,|and/).map(function(t){return t.trim();}).filter(function(t){return t.length > 0;});
            session.current_item.toppings = tops.map(function(t){return {name:t};});
          }
        } else if (field === "fixins") {
          var fixList = val.split(/,|and/).map(function(t){return t.trim();}).filter(function(t){return t.length > 0;});
          session.current_item.fixins = fixList;
        } else {
    ...
    if (intent === "done_ordering" || intent === "confirm" || 
      ...
      // Check for deals before confirming
      var deal = checkDeals(session.basket, session.declined_deals);
      if (deal.type === "near" && session.suggested_deals.indexOf(deal.match) === -1) {
        session.suggested_deals.push(deal.match);
        session.pending_deal = deal;
        session.state = "suggesting_deal";
        response.say = deal.suggestion;
        return response;
      }
      if (deal.type === "exact" && session.suggested_deals.indexOf(deal.match) === -1) {
        session.suggested_deals.push(deal.match);
        session.pending_deal = deal;
        session.state = "suggesting_deal";
        response.say = "I can make that a " + deal.match.replace(/_/g, " ") + ". Want that?";
        return response;
      }
      session.state = "confirming";
    ...
    if (intent === "ask_specials" || intent === "ask_price") {
      response.say = "We have the double deal, family deal with pizza and wings, sub combo, wings special, combo deal, and more. What would you like?";
      return response;
    }
    ...
    if (intent === "order_deal" && input.deal_name) {
      var resolved = resolveDealName(input.deal_name);
      if (!resolved) {
        response.say = "I'm not sure which deal that is. We have the double deal, family deal, sub combo, wings special, combo deal, burger combo, and fish combo. Which one?";
        return response;
      }
      session.current_deal_order = resolved;
      session.deal_components = {};
      session.state = "collecting_deal_components";
      // Tell customer what's included and ask first question
      response.say = resolved.description + " " + getNextDealQuestion(resolved, session.deal_components);
      return response;
    }
  ...
  if (state === "suggesting_deal") {
    if (intent === "confirm" || utterance.toLowerCase().match(/\b(yes|yeah|sure|ok|yep)\b/)) {
      // Customer accepted deal
      var dealType = session.pending_deal.match;
      session.current_deal = dealType;
      session.deal_fields = {};
      
      // Determine what extra fields this deal needs
      if (dealType.indexOf("near_pw") !== -1 || dealType === "pizza_wings") {
        session.deal_needed = ["two_liter_flavor"];
        session.state = "collecting_deal";
        response.say = "What flavor two-liter?";
      } else if (dealType.indexOf("near_combo") !== -1 || dealType === "combo_deal") {
        session.deal_needed = ["soda_flavor", "soda_flavor_2"];
        session.state = "collecting_deal";
        response.say = "What flavor for the first soda?";
      } else if (dealType.indexOf("near_dd") !== -1 || dealType === "double_deal") {
        // No extra fields needed
        session.deal_needed = [];
        session.state = "collecting_deal";
        var dealResult = buildDealFromBasket(session);
        if (dealResult.error) {
          response.say = dealResult.error;
          session.state = "collecting";
        } else {
          session.basket = dealResult.basket;
          response.say = "Got it. Anything else?";
          session.state = "collecting";
        }
        session.current_deal = null;
        session.deal_fields = null;
        session.pending_deal = null;
      } else {
        // Most deals need just soda flavor
        session.deal_needed = ["soda_flavor"];
        session.state = "collecting_deal";
  ...
  // ── COLLECTING DEAL COMPONENTS (ordered by name) ──
  if (state === "collecting_deal_components") {
    var dealDef = session.current_deal_order;
    if (!dealDef) { session.state = "collecting"; response.say = "What can I get for you?"; return response; }
    
    // Handle questions about the deal
    if (intent === "ask_specials" || intent === "ask_price" || utterance.toLowerCase().indexOf("what") !== -1 && utterance.toLowerCase().indexOf("include") !== -1) {
      response.say = dealDef.description + " " + getNextDealQuestion(dealDef, session.deal_components);
      return response;
    }
    
    // Handle cancel
    if (intent === "deny" || utterance.toLowerCase().match(/\b(cancel|never\s*mind|forget it)\b/)) {
      session.state = "collecting";
      session.current_deal_order = null;
      session.deal_components = null;
      response.say = "No problem. What can I get for you?";
      return response;
    }
    
    // Fill in the next needed field
    var val = answer || utterance.trim();
    if (val) {
      for (var di = 0; di < dealDef.needs.length; di++) {
        var field = dealDef.needs[di];
        if (!session.deal_components[field]) {
          session.deal_components[field] = val;
          break;
        }
      }
    }
    ...
    var nextQ = getNextDealQuestion(dealDef, session.deal_components);
    if (nextQ) {
      response.say = nextQ;
      return response;
    }
    
    // All collected — build deal
    var result = buildDealFromComponents(session);
    if (result.error) {
      response.say = result.error + " What can I get for you?";
      session.state = "collecting";
    } else {
      session.basket.push(result.item);
      response.say = "Got it. Anything else?";
      session.state = "collecting";
    }
    session.current_deal_order = null;
  ...
  // ── COLLECTING DEAL (near-match from basket) ──
  if (state === "collecting_deal") {
    var dealType = session.current_deal || "";
    var val = answer || utterance.trim();
    
    // Initialize deal fields if not set
    if (!session.deal_fields) {
      session.deal_fields = {};
      // Determine what fields this deal needs
      if (dealType.indexOf("near_sc") !== -1 || dealType.indexOf("sub_combo") !== -1 ||
          dealType.indexOf("near_wbuf") !== -1 || dealType.indexOf("near_wwhole") !== -1 ||
          dealType.indexOf("near_bc") !== -1 || dealType.indexOf("near_fc") !== -1 ||
          dealType.indexOf("near_ps") !== -1 || dealType.indexOf("wings_special") !== -1 ||
          dealType.indexOf("burger_combo") !== -1 || dealType.indexOf("fish_combo") !== -1 ||
          dealType.indexOf("pizza_sub") !== -1) {
        session.deal_needed = ["soda_flavor"];
      } else if (dealType.indexOf("near_pw") !== -1 || dealType.indexOf("pizza_wings") !== -1) {
        session.deal_needed = ["two_liter_flavor"];
      } else if (dealType.indexOf("near_combo") !== -1 || dealType.indexOf("combo_deal") !== -1) {
        session.deal_needed = ["soda_flavor", "soda_flavor_2"];
      } else {
        session.deal_needed = [];
      }
    ...
    // Fill in the next needed field
    if (session.deal_needed.length > 0 && val) {
      var field = session.deal_needed[0];
      session.deal_fields[field] = val;
      session.deal_needed.shift();
    }
    
    // Still need more fields?
    if (session.deal_needed.length > 0) {
      var nextField = session.deal_needed[0];
      if (nextField === "soda_flavor_2") response.say = "And the second soda flavor?";
      else if (nextField === "two_liter_flavor") response.say = "What flavor two-liter?";
      else if (nextField === "soda_flavor") response.say = "What flavor soda?";
      else response.say = "What would you like for " + nextField + "?";
      return response;
    }
    
    // All fields collected — build and price the deal
    var dealResult = buildDealFromBasket(session);
    if (dealResult.error) {
      response.say = dealResult.error + " Anything else?";
      session.state = "collecting";
    } else {
      session.basket = dealResult.basket;
      response.say = "Got it. Anything else?";
...
function processItems(session, items, response) {
  ...
  // Split items with quantity > 1 into separate items
  var expanded = [];
  for (var x = 0; x < items.length; x++) {
    var qty = parseInt(items[x].quantity) || 1;
    if (qty > 1 && (items[x].item_type === "pizza" || items[x].item_type === "sub" || items[x].item_type === "wrap")) {
      // Multiple items: process first, queue rest
      for (var q = 0; q < qty; q++) {
        var copy = JSON.parse(JSON.stringify(items[x]));
        copy.quantity = 1;
        if (items[x].item_type === "pizza") {
          copy._pizza_num = q + 1;
          copy._pizza_total = qty;
        }
        expanded.push(copy);
      }
    } else {
      expanded.push(items[x]);
    }
  }
  items = expanded;
...
function getMissingFields(item) {
  ...
  if (type === "pizza") {
    ...
    if (!item.toppings || item.toppings.length === 0) missing.push("toppings");
  }
  else if (type === "sub") {
...
function getFieldQuestion(field, item) {
  ...
  switch (field) {
    ...
    case "toppings": return item._pizza_num ? "What toppings on pizza " + item._pizza_num + " of " + item._pizza_total + "?" : "What toppings, or just cheese?";
...
// ═══════════════════════════════════════════════════════════════
// DEAL KNOWLEDGE — what each deal includes and what to collect
// ═══════════════════════════════════════════════════════════════
var DEAL_DEFINITIONS = {
  "double deal": {deal_type:"double deal",description:"The double deal is two same-size cheese pizzas with one free topping each.",needs:["pizza_size","pizza1_toppings","pizza2_toppings"],questions:{pizza_size:"What size — medium, large, or x-large?",pizza1_toppings:"What topping on the first pizza? You get one free.",pizza2_toppings:"And what topping on the second pizza?"}},
  "3 pizza special": {deal_type:"3 pizza special",description:"Three same-size pizzas with one free topping each plus a two-liter.",needs:["pizza_size","pizza1_toppings","pizza2_toppings","pizza3_toppings","two_liter_flavor"],questions:{pizza_size:"What size — large or x-large?",pizza1_toppings:"Topping on the first pizza?",pizza2_toppings:"Topping on the second?",pizza3_toppings:"And the third?",two_liter_flavor:"What flavor two-liter?"}},
  "family deal": {deal_type:"pizza and wings",description:"The family deal is a pizza with one free topping, ten buffalo wings, and a two-liter.",needs:["pizza_size","pizza_topping","wing_flavor","wing_dressing","two_liter_flavor"],questions:{pizza_size:"What size pizza — medium, large, or x-large?",pizza_topping:"What topping? You get one free.",wing_flavor:"What flavor for the wings?",wing_dressing:"Ranch or blue cheese?",two_liter_flavor:"What flavor two-liter?"}},
  "pizza and wings": {deal_type:"pizza and wings",description:"The family deal is a pizza with one free topping, ten buffalo wings, and a two-liter.",needs:["pizza_size","pizza_topping","wing_flavor","wing_dressing","two_liter_flavor"],questions:{pizza_size:"What size pizza?",pizza_topping:"What topping?",wing_flavor:"What flavor for the wings?",wing_dressing:"Ranch or blue cheese?",two_liter_flavor:"What flavor two-liter?"}},
  "pizza and sub": {deal_type:"pizza and sub",description:"Pizza with one free topping, a half sub, fries, and a can of soda.",needs:["pizza_size","pizza_topping","sub_name","sub_fixins","sub_cheese","soda_flavor"],questions:{pizza_size:"What size pizza?",pizza_topping:"What topping?",sub_name:"Which sub?",sub_fixins:"What do you want on it?",sub_cheese:"What cheese — American, provolone, or mozzarella?",soda_flavor:"What flavor soda?"}},
  "combo deal": {deal_type:"combo deal",description:"The combo deal is a pizza with one free topping, a half sub, buffalo wings, and two cans of soda. Twelve inch comes with six wings, fourteen inch with eight.",needs:["pizza_size","pizza_topping","sub_name","sub_fixins","sub_cheese","wing_flavor","wing_dressing","soda_flavor","soda_flavor_2"],questions:{pizza_size:"Twelve inch with six wings, or fourteen inch with eight?",pizza_topping:"What topping? You get one free.",sub_name:"Which half sub?",sub_fixins:"What do you want on it?",sub_cheese:"What cheese — American, provolone, or mozzarella?",wing_flavor:"What flavor for the wings?",wing_dressing:"Ranch or blue cheese?",soda_flavor:"First soda flavor?",soda_flavor_2:"And the second soda?"}},
  "sub combo": {deal_type:"sub combo",description:"A sub with fries and a can of soda.",needs:["sub_size","sub_name","sub_fixins","sub_cheese","soda_flavor"],questions:{sub_size:"Half or whole?",sub_name:"Which sub?",sub_fixins:"What do you want on it?",sub_cheese:"What cheese — American, provolone, or mozzarella?",soda_flavor:"What flavor soda?"}},
  "wings special": {deal_type:"wings special buffalo",description:"Six buffalo wings with fries and a can of soda.",needs:["wing_flavor","wing_dressing","soda_flavor"],questions:{wing_flavor:"What flavor?",wing_dressing:"Ranch or blue cheese?",soda_flavor:"What flavor soda?"}},
  "wings special buffalo": {deal_type:"wings special buffalo",description:"Six buffalo wings with fries and a can of soda.",needs:["wing_flavor","wing_dressing","soda_flavor"],questions:{wing_flavor:"What flavor?",wing_dressing:"Ranch or blue cheese?",soda_flavor:"What flavor soda?"}},
  "burger combo": {deal_type:"burger combo",description:"Cheeseburger sandwich with fries and a can of soda.",needs:["soda_flavor"],questions:{soda_flavor:"What flavor soda?"}},
  "fish combo": {deal_type:"fish combo",description:"Cheese fish sandwich with fries and a can of soda.",needs:["soda_flavor"],questions:{soda_flavor:"What flavor soda?"}}
};

function resolveDealName(input) {
  var nm = (input || "").toLowerCase().trim();
  if (DEAL_DEFINITIONS[nm]) return DEAL_DEFINITIONS[nm];
  var aliases = {"double":"double deal","dd":"double deal","two pizza":"double deal","3 pizza":"3 pizza special","three pizza":"3 pizza special","family":"family deal","pizza wings":"pizza and wings","pizza and wings":"pizza and wings","pizza sub":"pizza and sub","pizza and sub":"pizza and sub","combo":"combo deal","combo deal":"combo deal","the combo":"combo deal","sub combo":"sub combo","sub deal":"sub combo","wing special":"wings special","wings special":"wings special","wing deal":"wings special","burger":"burger combo","burger deal":"burger combo","burger combo":"burger combo","fish":"fish combo","fish deal":"fish combo","fish combo":"fish combo"};
  for (var key in aliases) {
    if (nm.indexOf(key) !== -1) return DEAL_DEFINITIONS[aliases[key]] || null;
  }
  return null;
}

function getNextDealQuestion(dealDef, collected) {
  for (var i = 0; i < dealDef.needs.length; i++) {
    if (!collected[dealDef.needs[i]]) return dealDef.questions[dealDef.needs[i]] || "What for " + dealDef.needs[i] + "?";
  }
  return null;
}

function buildDealFromComponents(session) {
  var dealDef = session.current_deal_order;
  var c = session.deal_components;
  var dealType = dealDef.deal_type;
  var dealInfo = MENU.deals[dealType];
  if (!dealInfo) return {error: "Deal not found."};
  var components = [];
  var sz = resolveSize(c.pizza_size) || 0;
  // Pizzas
  if (sz > 0) {
    var pid = MENU.pizzas[0].ids[sz] || 737;
    var labels = {10:"Sm",12:"Med",14:"Lrg",16:"XL",18:"XXL"};
    var pName = sz + " inch " + (labels[sz]||"Lrg") + " Cheese Pizza";
    var pizzaTops = [];
    for (var k = 1; k <= 4; k++) { if (c["pizza"+k+"_toppings"]) pizzaTops.push(c["pizza"+k+"_toppings"]); }
    if (c.pizza_topping) pizzaTops = [c.pizza_topping];
    if (pizzaTops.length === 0) pizzaTops = ["cheese"];
    for (var p = 0; p < (dealInfo.pizzas || 1); p++) {
      var topName = pizzaTops[p] || pizzaTops[0] || "cheese";
      var mods = [];
      if (topName !== "cheese" && topName !== "just cheese") {
        mods.push({name:topName,modifier_id:(MENU.topping_ids[sz]||{})[norm(topName)]||null,group:"Add Toppings",price:0,charge:0});
      }
      components.push({component_name:pName,item_id:String(pid),category:"Pizzas",modifiers:mods});
    }
  }
  // Sub
  if (c.sub_name) {
    var sm = matchKeyword(c.sub_name, MENU.subs);
    if (sm) {
      var ssz = resolveSize(c.sub_size) || 8;
      var subMods = [];
      if (c.sub_fixins) subMods.push({name:c.sub_fixins,modifier_id:MENU.sub_fixins[norm(c.sub_fixins)]||463,group:"Sub Fixins",price:0});
      if (c.sub_cheese) subMods.push({name:c.sub_cheese,modifier_id:MENU.cheese_options[norm(c.sub_cheese)]||null,group:"Cheese Options",price:0});
      components.push({component_name:(ssz===12?"12 inch Whole":"8 inch Half")+" "+sm.name,item_id:String(sm.id),category:"Submarines",modifiers:subMods});
    }
  }
  // Wings
  if (c.wing_flavor || dealInfo.wings) {
    var wq = dealInfo.wings || 6;
    var wi = MENU.buffalo_wings.find(function(w){return w.qty===wq;});
    if (wi) {
      var wm = [];
      if (c.wing_flavor) wm.push({name:c.wing_flavor,modifier_id:MENU.wing_flavors[norm(c.wing_flavor)]||null,group:"Wings - Flavors",price:0});
      if (c.wing_dressing) wm.push({name:c.wing_dressing,modifier_id:MENU.wing_dressings[norm(c.wing_dressing)]||null,group:"Wings - Dressing",price:0});
      components.push({component_name:wq+"pcs Buffalo wings",item_id:String(wi.id),category:"Wings",modifiers:wm});
    }
  }
  // Fries
  if (dealInfo.includes_fries) components.push({component_name:"French Fries",item_id:"607",category:"Sides"});
  // Sodas
  if (c.soda_flavor) components.push({component_name:"Can Soda",item_id:"728",category:"Beverages",modifiers:[{name:c.soda_flavor,modifier_id:MENU.soda_flavors[norm(c.soda_flavor)]||null,group:"Soda Can",price:0}]});
  if (c.soda_flavor_2) components.push({component_name:"Can Soda",item_id:"728",category:"Beverages",modifiers:[{name:c.soda_flavor_2,modifier_id:MENU.soda_flavors[norm(c.soda_flavor_2)]||null,group:"Soda Can",price:0}]});
  if (c.two_liter_flavor) components.push({component_name:"2 Liter Soda",item_id:"790",category:"Beverages",modifiers:[{name:c.two_liter_flavor,modifier_id:MENU.soda_flavors[norm(c.two_liter_flavor)]||null,group:"Soda 2 Liter",price:0}]});
  // Burger/fish
  if (dealType === "burger combo") components.unshift({component_name:"Cheese Burger Sandwich",item_id:"713",category:"Sandwiches",modifiers:[]});
  if (dealType === "fish combo") components.unshift({component_name:"Cheese Fish Sandwich",item_id:"718",category:"Sandwiches",modifiers:[]});
  // Price
  var sizeKey = sz || 0;
  var dp = dealInfo.sizes[sizeKey] || dealInfo.sizes[Object.keys(dealInfo.sizes)[0]];
  if (!dp) return {error: "No price for that size."};
  var dealName = dealType.split(" ").map(function(w){return w.charAt(0).toUpperCase()+w.slice(1);}).join(" ");
  if (sz) dealName += " " + sz + " inch";
  return {item:{item_name:dealName,item_id:String(dp.id),unit_price:dp.price,category:"Combo",quantity:1,is_combo:true,deal_id:String(dp.id),components:components}};
}

// ── Build a deal from basket items + collected fields ──
function buildDealFromBasket(session) {
  var dealType = session.current_deal || "";
  var fields = session.deal_fields || {};
  var basket = session.basket;
  
  // Map near-match to actual deal type
  var dealMap = {
    "near_sc": "sub combo", "near_wbuf": "wings special buffalo", "near_wwhole": "wings special whole",
    "near_bc": "burger combo", "near_fc": "fish combo", "near_ps": "pizza and sub",
    "near_pw": "pizza and wings", "near_dd": "double deal", "near_combo": "combo deal",
    "sub_combo": "sub combo", "wings_special_buffalo": "wings special buffalo",
    "wings_special_whole": "wings special whole", "burger_combo": "burger combo",
    "fish_combo": "fish combo", "pizza_sub": "pizza and sub",
    "pizza_wings": "pizza and wings", "double_deal": "double deal", "combo_deal": "combo deal"
  };
  
  var actualDeal = dealMap[dealType] || dealType;
  var dealInfo = MENU.deals[actualDeal];
  if (!dealInfo) return {error: "Deal not found: " + actualDeal};
  ...
  // Find pizza(s)
  for (var i = 0; i < basket.length; i++) {
    if (consumedIndices.indexOf(i) !== -1) continue;
    var it = basket[i];
    if (it.category === "Pizzas") {
      components.push({component_name: it.item_name, item_id: String(it.item_id), category: "Pizzas", modifiers: it.modifiers || []});
      if (it.special_instructions) components[components.length-1].special_instructions = it.special_instructions;
  ...
  for (var i = 0; i < basket.length; i++) {
    if (consumedIndices.indexOf(i) !== -1) continue;
    var it = basket[i];
    if (it.category === "Submarines") {
      components.push({component_name: it.item_name, item_id: String(it.item_id), category: "Submarines", modifiers: it.modifiers || []});
      if (it.special_instructions) components[components.length-1].special_instructions = it.special_instructions;
      consumedIndices.push(i);
    }
  }
  ...
  for (var i = 0; i < basket.length; i++) {
    if (consumedIndices.indexOf(i) !== -1) continue;
    var it = basket[i];
    if (it.category === "Wings") {
      components.push({component_name: it.item_name, item_id: String(it.item_id), category: "Wings", modifiers: it.modifiers || []});
      consumedIndices.push(i);
    }
  }
  
  // Find existing fries/soda
  for (var i = 0; i < basket.length; i++) {
    if (consumedIndices.indexOf(i) !== -1) continue;
    var it = basket[i];
    var nm = (it.item_name || "").toLowerCase();
    if (nm.indexOf("fries") !== -1 || nm.indexOf("can soda") !== -1 || nm.indexOf("2 liter") !== -1) {
      consumedIndices.push(i);
    }
  }
  
  // Add fries if deal includes it
  if (dealInfo.includes_fries) {
    components.push({component_name: "French Fries", item_id: "607", category: "Sides"});
  }
  
  // Add soda
  if (dealInfo.includes_can || fields.soda_flavor) {
    var sodaFlavor = fields.soda_flavor || "Coke";
    var sn = norm(sodaFlavor);
    components.push({component_name: "Can Soda", item_id: "728", category: "Beverages",
      modifiers: [{name: sodaFlavor, modifier_id: MENU.soda_flavors[sn] || null, group: "Soda Can", price: 0}]});
  }
  if (fields.soda_flavor_2) {
    var sn2 = norm(fields.soda_flavor_2);
    components.push({component_name: "Can Soda", item_id: "728", category: "Beverages",
      modifiers: [{name: fields.soda_flavor_2, modifier_id: MENU.soda_flavors[sn2] || null, group: "Soda Can", price: 0}]});
  }
  if (dealInfo.includes_2l || fields.two_liter_flavor) {
    var tlFlavor = fields.two_liter_flavor || "Coke";
    var tn = norm(tlFlavor);
    components.push({component_name: "2 Liter Soda", item_id: "790", category: "Beverages",
      modifiers: [{name: tlFlavor, modifier_id: MENU.soda_flavors[tn] || null, group: "Soda 2 Liter", price: 0}]});
  }
  
  // Determine deal size and price
  var sz = 0;
  if (components.length > 0) {
    var firstComp = components[0].component_name || "";
    var szMatch = firstComp.match(/(\d+)\s*inch/);
    if (szMatch) sz = parseInt(szMatch[1]);
  }
  
  var sizeKey = sz || 0;
  var dealPrice = dealInfo.sizes[sizeKey] || dealInfo.sizes[Object.keys(dealInfo.sizes)[0]];
  if (!dealPrice) return {error: "No price for deal size " + sizeKey};
  
  // Calculate extra charges (toppings beyond free)
  var extraCharge = 0;
  var freeTop = dealInfo.freeToppings || 0;
  if (freeTop > 0) {
    for (var c = 0; c < components.length; c++) {
      if (components[c].category === "Pizzas" && components[c].modifiers) {
        var topCount = 0;
        for (var m = 0; m < components[c].modifiers.length; m++) {
          if (components[c].modifiers[m].group === "Add Toppings") {
            topCount++;
            if (topCount > freeTop) {
              var topPrice = (MENU.topping_prices[sz] || MENU.topping_prices[14]).full;
              extraCharge += topPrice;
              components[c].modifiers[m].price = topPrice;
              components[c].modifiers[m].charge = topPrice;
            } else {
              components[c].modifiers[m].price = 0;
              components[c].modifiers[m].charge = 0;
            }
          }
        }
      }
  ...
  var unitPrice = round2(dealPrice.price + extraCharge);
  
  // Build the deal basket item
  var dealItem = {
    item_name: actualDeal.split(" ").map(function(w){return w.charAt(0).toUpperCase()+w.slice(1);}).join(" ") + (sz ? " " + sz + " inch" : ""),
    item_id: String(dealPrice.id),
    unit_price: unitPrice,
    category: "Combo",
    quantity: 1,
    is_combo: true,
    deal_id: String(dealPrice.id),
    components: components
  };
  
  // Build new basket: keep non-consumed items, add deal
  var newBasket = [];
  for (var i = 0; i < basket.length; i++) {
    if (consumedIndices.indexOf(i) === -1) newBasket.push(basket[i]);
  }
  newBasket.push(dealItem);
