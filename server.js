...
function buildRegularItem(item) {
  ...
  // Safety: clean topping words out of pizza item names
  var itemName = cleanItemName(rawName, mods);
...
app.get("/test-combo", async function (req, res) {
  // Double Deal: 2x 14" pizza (1 top each) = $21.99 [Disc:870]
  // Pizza 1: black olives (free) + onions (extra $2.00)
  // Pizza 2: tomatoes (free)
  var order = { order_type: "pickup", customer_name: "Test Combo", customer_phone: "8888888888",
    items: [
      { item_name: '14 inch Cheese Pizza', item_id: "737", quantity: 1, unit_price: 11.99, combo_group: 1,
        modifiers: [
          { group: "Add Toppings", name: "Black Olives", price: 0 },
          { group: "Add Toppings", name: "Onions", price: 2.00 }
        ] },
      { item_name: '14 inch Cheese Pizza', item_id: "737", quantity: 1, unit_price: 11.99, combo_group: 1,
        modifiers: [
          { group: "Add Toppings", name: "Tomatoes", price: 0 }
        ] },
    ],
    combo_deals: [
      { group: 1, name: "Double Deal 2x14", id: "870", price: 21.99 }
    ],
    subtotal: 23.99, tax: 1.44, total: 25.43 };
  var built = buildXml(order);
  var result = await sendToPOS(built.xml);
  res.json({ version: "V16", note: "Double Deal: 2x14\" pizza. Olives(free)+Onions(+$2) on 1st, Tomatoes(free) on 2nd. Discount auto-calc.", xml: built.xml, pos: result, ref: built.ref });
});

app.get("/test-combo2", async function (req, res) {
  // Pizza Sub Combo: 14" pizza + 8" sub + fries + can = $24.99 [Disc:873]
  var order = { order_type: "pickup", customer_name: "Test PSC", customer_phone: "8888888888",
    items: [
      { item_name: '14 inch Cheese Pizza', item_id: "737", quantity: 1, unit_price: 11.99, combo_group: 1,
        modifiers: [{ group: "Add Toppings", name: "Pepperoni", price: 0 }] },
    ...
    combo_deals: [
      { group: 1, name: "Pizza Sub Combo 14", id: "873", price: 24.99 }
    ],
    subtotal: 24.99, tax: 1.50, total: 26.49 };
  var built = buildXml(order);
  var result = await sendToPOS(built.xml);
  res.json({ version: "V16", note: "Pizza+Sub Combo 14\". All items at regular price, discount auto-calc.", xml: built.xml, pos: result, ref: built.ref });
});
...
app.get("/", function (req, res) {
  res.json({ status: "running", version: "V16", restaurant_id: CONFIG.RESTAURANT_ID,
    ...
    endpoints: ["POST /retell/function/submit_order", "POST /retell/function/verify_address", "POST /retell/function/process_payment", "POST /retell/function/get_caller_id",
      ...
      "GET /test-order", "GET /test-combo (Double Deal)", "GET /test-combo2 (Pizza Sub)", "GET /test-delivery", "GET /test-payment", "GET /debug"] });
});
