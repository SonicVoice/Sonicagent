...
// Build combo: single OrderLineItem with deal name + price, all components listed as Requirements underneath
// No duplicate component lines — everything shows once under the combo name
function buildComboItems(item) {
  var o = "";

  // Single OrderLineItem: combo deal name with full price, all components as Requirements underneath
  o += "<OrderLineItem>";
  ...
  if (item.components && item.components.length > 0) {
    ...
    for (var c = 0; c < item.components.length; c++) {
      ...
      // Component header (e.g., "14 inch Lrg Cheese Pizza", "8 inch Half Cheese Steak Sub")
      o += "<Requirement>";
      ...
      if (comp.modifiers && comp.modifiers.length > 0) {
        for (var m = 0; m < comp.modifiers.length; m++) {
          ...
          var modPrice = parseFloat(mod.price || 0);
          // Build modifier display name with charge if applicable
