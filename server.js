...
function buildRegularItem(item) {
  ...
  if (item.modifiers && item.modifiers.length > 0) {
    ...
    for (var i = 0; i < item.modifiers.length; i++) {
      ...
      var modDisplay = mod.name || mod;
      if (modPrice > 0) {
        modDisplay += " +$" + modPrice.toFixed(2);
      }
      o += "<Requirement>";
      if (mod.group) o += "<groupMame>" + X(mod.group) + "</groupMame>";
      o += NT + X(modDisplay) + NTC;
...
function buildComboItems(item) {
  ...
  // Single OrderLineItem: combo deal name with price, all components as Requirements underneath
  o += "<OrderLineItem>";
  ...
  // All components and their modifiers listed as Requirements under the combo
  if (item.components && item.components.length > 0) {
    o += "<Requirements>";
    for (var c = 0; c < item.components.length; c++) {
  ...
  // NO duplicate component OrderLineItems — everything is under the single combo line above
  return o;
}
...
var SUBS_DB = [
  ...
  {keys:["regular cold cut","american cold cut"], id:825, name:"Regular Cold Cut Sub", p8:7.49, p12:10.99},
  {keys:["regular hot cut","american hot cut"], id:826, name:"Regular Hot Cut Sub", p8:7.49, p12:10.99},
  ...
  try {
    ...
    if (itemType === "sandwich") {
      ...
      for (var sf = 0; sf < fixins.length; sf++) {
        var sfn = normTopping(fixins[sf]);
        var sfMatch = sandFixinsDB ? sandFixinsDB.find(function(x) { return normTopping(x.fixin_name) === sfn; }) : null;
    ...
    if (itemType === "wrap") {
      ...
      for (var wf = 0; wf < fixins.length; wf++) {
        var wfn = normTopping(fixins[wf]);
        var wfMatch = wrapFixinsDB ? wrapFixinsDB.find(function(x) { return normTopping(x.fixin_name) === wfn; }) : null;
    ...
    if (itemType === "wings" || itemType === "buffalo wings" || itemType === "whole wings") {
      ...
      if (flavor) {
        ...
        var flMatch = flavorDB ? flavorDB.find(function(x) { return x.flavor_name.toLowerCase() === flavor.toLowerCase(); }) : null;
      ...
      if (dressing) {
        ...
        var drMatch = dressDB ? dressDB.find(function(x) { return x.dressing_name.toLowerCase() === dressing.toLowerCase(); }) : null;
    ...
    if (itemType === "salad") {
      ...
      if (saladDress) {
        ...
        var sdMatch = sdDB ? sdDB.find(function(x) { return x.dressing_name.toLowerCase() === saladDress.toLowerCase(); }) : null;
      ...
      // Salad extras (from Group 67 in POS)
      var SALAD_EXTRAS = {
    ...
    if (itemType === "gyro") {
      ...
      for (var gf = 0; gf < fixins.length; gf++) {
        var gfn = normTopping(fixins[gf]);
        var gfMatch = gyroFixinsDB ? gyroFixinsDB.find(function(x) { return normTopping(x.fixin_name) === gfn; }) : null;
    ...
    if (itemType === "tenders" || itemType === "chicken tenders") {
      ...
      if (sauce) {
        ...
        var tsMatch = tsDB ? tsDB.find(function(x) { return x.sauce_name.toLowerCase() === sauce.toLowerCase(); }) : null;
    ...
    if (itemType === "drink" || itemType === "beverage" || itemType === "soda") {
      ...
      if (sodaFl) {
        ...
        var sfMatch = sfDB ? sfDB.find(function(x) { return x.flavor_name.toLowerCase() === sodaFl.toLowerCase(); }) : null;
    ...
    if (itemType === "stromboli") {
      ...
      var stResult = buildModifiers(toppings, 12, 0); // use 12" topping prices, 0 free
      var stPrice = stBase + stResult.extra_charge;
    ...
    if (itemType === "quesadilla") {
      ...
      var qResult = buildModifiers(toppings, 12, 0); // use 12" topping prices, 0 free
      var qPrice = qBase + qResult.extra_charge;
...
app.post("/retell/function/calculate_combo", function (req, res) {
  try {
    ...
    var PREMIUM_SUB_IDS = [723, 724, 725, 726, 710]; // Philly CS, Philly Chicken, CS Special, Shrimp CS, Shrimp Salad
    var PREMIUM_EXCLUDED_DEALS = ["pizza and sub", "pizza sub combo", "combo deal", "pizza sub wings", "wings and sub", "wings sub combo"];

    // Check if sub is premium and deal excludes it
    if (subName && PREMIUM_EXCLUDED_DEALS.indexOf(dealType) !== -1) {
      ...
      if (premCheck && PREMIUM_SUB_IDS.indexOf(premCheck.id) !== -1) {
        return res.json("ERROR: " + premCheck.name + " is a premium sub and cannot be used in " + dealType + ". Only regular subs qualify for this deal. This sub can be made into a Sub Combo (sub + fries + can soda) instead. Use deal_type 'sub combo'.");
      }
    }
    ...
    if (deal.includes_fries) {
      var FRIES_UPGRADES = {
        "western fries": {name:"Western Fries Large", id:"608", price:2.00},
        "western": {name:"Western Fries Large", id:"608", price:2.00},
        "cheese fries": {name:"Fries with Cheese", id:"811", price:2.00},
        "fries with cheese": {name:"Fries with Cheese", id:"811", price:2.00},
        "nacho fries": {name:"Fries with Nacho", id:"611", price:2.00},
        "fries with nacho": {name:"Fries with Nacho", id:"611", price:2.00},
        "gravy fries": {name:"Fries with Gravy", id:"609", price:2.00},
        "fries with gravy": {name:"Fries with Gravy", id:"609", price:2.00},
        "mozzarella fries": {name:"Fries with Mozzarella", id:"612", price:2.00},
        "fries with mozzarella": {name:"Fries with Mozzarella", id:"612", price:2.00},
        "pizza fries": {name:"Pizza Fries", id:"615", price:2.00},
        "crazy fries": {name:"Crazy Fries", id:"613", price:2.00},
        "large fries": {name:"Large French Fries", id:"830", price:2.00}
      };
      ...
      if (friesUpgrade && FRIES_UPGRADES[friesUpgrade]) {
        ...
        friesComp = {component_name: upgrade.name + " (+$2.00)", item_id: upgrade.id, category: "Sides"};
        totalExtra += upgrade.price;
      }
