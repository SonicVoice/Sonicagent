...
// Kitchen 1 (printKitchen) = subs, wings, sandwiches, salads, wraps, pasta, seafood, sides, beverages, desserts, gyro, chicken
// Kitchen 2 (printKitchen2) = pizzas, stromboli
function isKitchen2(item) {
  var cat = (item.category || "").toLowerCase();
  if (cat === "pizzas" || cat === "stromboli") return true;
  // Fallback: detect from item name or ID
  var nm = (item.item_name || item.name || item.component_name || "").toLowerCase();
  if (nm.indexOf("pizza") !== -1 || nm.indexOf("stromboli") !== -1) return true;
  // Pizza IDs: 735-739 (cheese), 655-667,829 (specialty)
  var id = parseInt(item.item_id || 0);
  if ((id >= 735 && id <= 739) || (id >= 655 && id <= 667) || id === 829) return true;
  // Stromboli IDs: 668-671
  if (id >= 668 && id <= 671) return true;
  return false;
}

function getKitchenTags(item) {
  if (isKitchen2(item)) {
    return "<printKitchen>N</printKitchen><printKitchen2>Y</printKitchen2>";
  }
  return "<printKitchen>Y</printKitchen><printKitchen2>N</printKitchen2>";
}

function getComboKitchenTags(item) {
  // Check all components to determine which printers
  var hasK1 = false;
  var hasK2 = false;
  ...
  // Fallback: if no components or can't determine, check item name
  if (!hasK1 && !hasK2) {
    if (isKitchen2(item)) { hasK2 = true; }
    else { hasK1 = true; }
  }
  return "<printKitchen>" + (hasK1 ? "Y" : "N") + "</printKitchen>"
       + "<printKitchen2>" + (hasK2 ? "Y" : "N") + "</printKitchen2>";
}
...
function buildRegularItem(item) {
  ...
  // Kitchen printer routing based on category
  if (item.is_combo) {
    o += getComboKitchenTags(item);
  } else {
    o += getKitchenTags(item);
  }
...
function buildXml(order) {
  ...
  if (order.items && order.items.length > 0) {
    ...
    for (var di = 0; di < appliedDiscounts.length; di++) {
      ...
      o += "<printKitchen>N</printKitchen><printKitchen2>N</printKitchen2>";
      o += "</OrderLineItem>";
