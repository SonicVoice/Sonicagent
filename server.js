...
app.get("/test-order", async function (req, res) {
  ...
  var result = await sendToPOS(built.xml);
  res.json({ version: "V9", xml: built.xml, pos: result, ref: built.ref });
});
...
// ═══════════════════════════════════════════════════
// RAW XML VIEWER — shows actual XML tags without browser rendering
// ═══════════════════════════════════════════════════
app.get("/raw-xml", function (req, res) {
  var order = {
    order_type: "pickup", customer_name: "Raw Test", customer_phone: "8888888888",
    items: [{
      item_name: '12" PIZZA W/1 TOP, 8" SUB, 6 BUFFALO WINGS & 2 CAN SODA',
...
app.get("/raw-lastorder", function (req, res) {
  res.set("Content-Type", "text/plain; charset=utf-8");
  if (orderLog.length > 0 && orderLog[0].xml) {
    res.send(orderLog[0].xml);
  } else {
...
app.get("/", function (req, res) {
  res.json({
    ...
    endpoints: [
      ...
      "GET /test-order",
      "GET /test-combo",
      "GET /raw-xml (shows actual XML tags as plain text)",
      "GET /raw-lastorder (shows last Retell order XML as plain text)",
      "GET /debug",
    ],
  });
});
...
app.listen(CONFIG.PORT, function () {
  console.log("V9 POS Bridge | Port: " + CONFIG.PORT + " | ID: " + CONFIG.RESTAURANT_ID);
});
