...
function buildXml(order) {
  ...
  if (order.items && order.items.length > 0) {
    ...
    // COMBO DEALS: add discount line items using FIXED XML prices
    if (order.combo_deals && order.combo_deals.length > 0) {
      for (var cd = 0; cd < order.combo_deals.length; cd++) {
        ...
        // Use FIXED discount amount from XML (sent by agent as negative number)
        // Falls back to dynamic calculation if agent sends old format with 'price'
        var discountAmt = 0;
        if (deal.discount && deal.discount < 0) {
          // New format: agent sends fixed negative discount from XML
          discountAmt = Math.abs(parseFloat(deal.discount));
        } else if (deal.discount && deal.discount > 0) {
          // Agent sent positive number, make it negative
          discountAmt = parseFloat(deal.discount);
        } else if (deal.price || deal.combo_price) {
          // Old format fallback: calculate discount = regularTotal - comboPrice
          ...
          discountAmt = Math.round((regularTotal - comboPrice) * 100) / 100;
        }

        if (discountAmt > 0) {
          o += "<OrderLineItem>";
          o += "<itemName>" + X(comboName) + "</itemName>";
          if (comboId && /^\d+$/.test(String(comboId))) {
            o += "<foodMenuItemId>" + X(comboId) + "</foodMenuItemId>";
          }
          o += "<quantity>1</quantity>";
          o += "<unitPrice>-" + discountAmt.toFixed(2) + "</unitPrice>";
          o += "<printKitchen>N</printKitchen>";
          o += "</OrderLineItem>";
        }

        console.log("COMBO: " + comboName + " | ID: " + comboId + " | Discount: -$" + discountAmt.toFixed(2));
      }
    }
  }
  o += "</Items></FoodOrder>";
...
app.get("/", function (req, res) {
  res.json({ status: "running", version: "V16", restaurant_id: CONFIG.RESTAURANT_ID,
    ...
    endpoints: ["POST /retell/function/submit_order", "POST /retell/function/verify_address", "POST /retell/function/process_payment", "POST /retell/function/get_caller_id",
      ...
      "GET /test-order", "GET /test-combo (Double Deal)", "GET /test-combo2 (Pizza Sub)", "GET /test-3pizza (3 Pizza Special)", "GET /test-delivery", "GET /test-payment", "GET /debug"] });
});
